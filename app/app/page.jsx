...
export default function Page() {
  ...
  const [searchStory, setSearchStory] = useState(null);
  const [searchStatus, setSearchStatus] = useState("idle"); // idle | loading | fetching | low-coverage | error
  ...
  async function handleSearch(fresh = false) {
    const q = searchInput.trim();
    if (!q || searchStatus === "loading" || searchStatus === "fetching") return;
    setSearchQuery(q);
    setSearchStory(null);
    setSelectedId(null);
    setSearchStatus(fresh ? "fetching" : "loading");
    try {
      const res = await fetch("/api/search", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ query: q, fresh }),
      });
      const data = await res.json();
      if (data.coverage === "low") {
        setSearchStatus("low-coverage");
      } else if (data.story) {
        setSearchStory({
          ...data.story,
          quadrants: Object.fromEntries(
            Object.entries(data.story.quadrants).map(([key, val]) => [key, { ...val, ...QUADRANT_META[key] }])
          ),
        });
  ...
  const story = selectedId !== null
    ? (stories.find((s) => s.id === selectedId) || null)
    : searchStory;
  const isSearchResult = selectedId === null && searchStory !== null;
        ...
        {!story ? (
          <div>
            ...
            <div style={{ margin: "24px 0 0" }}>
              <form onSubmit={(e) => { e.preventDefault(); handleSearch(false); }} style={{ display: "flex", gap: 8 }}>
                <input
                  value={searchInput}
                  onChange={(e) => setSearchInput(e.target.value)}
                  placeholder="Search any topic…"
                  disabled={searchStatus === "loading" || searchStatus === "fetching"}
                  style={{ flex: 1, fontFamily: "var(--font-inter), sans-serif", fontSize: 14, padding: "11px 16px", borderRadius: 10, border: "1px solid #EDEAE4", background: "#fff", color: "#1a1916", outline: "none" }}
                />
                <button
                  type="submit"
                  disabled={!searchInput.trim() || searchStatus === "loading" || searchStatus === "fetching"}
                  style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 13, fontWeight: 600, color: "#fff", background: searchInput.trim() && searchStatus !== "loading" && searchStatus !== "fetching" ? "#1a1916" : "#d0cbc5", border: "none", borderRadius: 10, padding: "11px 20px", cursor: searchInput.trim() && searchStatus !== "loading" && searchStatus !== "fetching" ? "pointer" : "default", transition: "background 0.15s", whiteSpace: "nowrap" }}
                >
                  Search
                </button>
              </form>
              {searchStatus === "low-coverage" && (
                <div style={{ marginTop: 12, padding: "14px 16px", background: "#FBF5E8", border: "1px solid #EDD8B8", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                  <p style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 13, color: "#956B2A", margin: 0 }}>
                    Not enough coverage on <strong>"{searchQuery}"</strong> in today's cached headlines.
                  </p>
                  <button
                    onClick={() => handleSearch(true)}
                    style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 12, fontWeight: 600, color: "#956B2A", background: "#FCF6EE", border: "1.5px solid #EDD8B8", borderRadius: 8, padding: "6px 14px", cursor: "pointer", whiteSpace: "nowrap" }}
                  >
                    Fetch latest on this →
                  </button>
                </div>
              )}
              {searchStatus === "error" && (
                <p style={{ marginTop: 10, fontFamily: "var(--font-inter), sans-serif", fontSize: 12, color: "#C47B3C" }}>
            ...
            <div style={{ padding: "32px 0 16px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 6 }}>
                <span style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.12em", textTransform: "uppercase", color: "#b0aba5" }}>
                  {loading || searchStatus === "loading" || searchStatus === "fetching" ? "Loading stories…" : "Latest stories"}
                </span>
            ...
            {(loading || searchStatus === "loading" || searchStatus === "fetching") && (
              <div style={{ textAlign: "center", padding: "40px 0", fontFamily: "var(--font-inter), sans-serif", fontSize: 13, color: "#b0aba5" }}>
                {searchStatus === "fetching"
                  ? `Fetching latest headlines on "${searchQuery}"… this takes about 15 seconds.`
                  : searchStatus === "loading"
                  ? `Searching today's headlines for "${searchQuery}"…`
                  : "Fetching today's stories…"}
              </div>
            )}

            {!loading && searchStatus !== "loading" && searchStatus !== "fetching" && stories.map((s) => {
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10 }}>
                        ...
                        <span style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 11, color: "#b0aba5" }}>{createdAt ? formatTime(createdAt) + " · " : ""}{s.date}</span>
                      </div>
          ...
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "24px 0 20px" }}>
              ...
              {isSearchResult && (
                <span style={{ fontFamily: "var(--font-inter), sans-serif", fontSize: 10, fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: "#b0aba5" }}>Search result</span>
              )}
            </div>
