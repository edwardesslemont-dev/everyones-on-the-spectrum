...
export async function generateSearchStory(headlines, query) {
  const headlineText = formatHeadlines(headlines);

  const prompt = `You are the editorial engine for "Everyone's on the Spectrum". A user wants to understand this specific topic: "${query}"

Here are the latest headlines from sources across the political spectrum:

${headlineText}

Generate ONE story about "${query}".
- Use any relevant headlines above as grounding where they apply. If few headlines directly mention the topic, draw on your knowledge of the current debate — the goal is an accurate, well-grounded analysis regardless of today's specific coverage.
- Neutral, story-level headline
- A category from: Politics, Economy, Technology, Health, World, Culture
- 4–6 facts grounded in current reality (specific events, numbers, names, recent developments)
- For each of the 4 quadrants: sentiment (1–5), why, defense, sources (2–3 real outlet names)
- Write with genuine empathy for each quadrant. Make the strongest honest version of each argument. Keep why and defense to 2–3 sentences — punchy, not exhaustive.
...

{
  ...
  "quadrants": {
    "authLeft":  { "sentiment": 3, "why": "...", "defense": "...", "sources": ["Source A", "Source B"] },
    "authRight": { "sentiment": 3, "why": "...", "defense": "...", "sources": ["Source A", "Source B"] },
    "libLeft":   { "sentiment": 3, "why": "...", "defense": "...", "sources": ["Source A", "Source B"] },
    "libRight":  { "sentiment": 3, "why": "...", "defense": "...", "sources": ["Source A", "Source B"] }
  }
}`;
  ...
  if (parsed.coverage === "low") return { coverage: "low" }; // fallback, shouldn't happen
  ...
  return {
    ...
    date: new Date().toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }),
    quadrants: Object.fromEntries(
      Object.entries(parsed.quadrants).map(([key, val]) => [
        key,
        { ...QUADRANT_META[key], ...val },
      ])
    ),
