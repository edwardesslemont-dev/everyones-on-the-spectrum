...
function getHistoryKey(headline) {
  const slug = headline.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "").slice(0, 80);
  return `dig-deeper-history-${slug}`;
}

function loadHistory(headline) {
  try {
    const raw = localStorage.getItem(getHistoryKey(headline));
    if (!raw) return null;
    const { messages, savedAt } = JSON.parse(raw);
    if (Date.now() - savedAt > HISTORY_TTL) {
      localStorage.removeItem(getHistoryKey(headline));
      return null;
    }
...
function saveHistory(headline, messages) {
  try {
    localStorage.setItem(getHistoryKey(headline), JSON.stringify({ messages, savedAt: Date.now() }));
  } catch {}
}
...
function ChatUI({ story, initialMessages }) {
  ...
  useEffect(() => {
    if (messages.length > 0) saveHistory(story.headline, messages);
  ...
  const sendMessage = useCallback(async (userText, currentMessages) => {
    ...
    try {
      ...
      saveHistory(story.headline, finalMessages);
    } catch (err) {
  ...
  function clearHistory() {
    ...
    localStorage.removeItem(getHistoryKey(story.headline));
    const empty = [];
    setMessages(empty);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        setStory(s);
        setInitialMessages(loadHistory(s.headline) ?? []);
      } else {
        setInitialMessages([]);
      }
    } catch {
